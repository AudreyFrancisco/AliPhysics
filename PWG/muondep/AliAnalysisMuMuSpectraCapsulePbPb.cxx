...
void AliAnalysisMuMuSpectraCapsulePbPb::DrawResults( const char* particle,const char* subresults) const
{
  ...
  while ((r = static_cast<AliAnalysisMuMuBinning::Range*>(nextBin())))
  {
    ...
    while ((sr = static_cast<AliAnalysisMuMuResult*>(nextSubResult())))
    {
      ...
      DeltaPhi.push_back ( r->Ymin() );
      DeltaPhi.push_back ( r->Ymax() );
  ...
  while ((h2 = static_cast<TH1*>(nextHisto())))
  {
    ...
    if (h){
      ...
      if(BinType.Contains("DPCORR")|BinType.Contains("DPHI")){
        if (n & 1)pt->AddText(Form(" %.2f < #Delta #Phi < %.2f (rad)",DeltaPhi[n-1],DeltaPhi[n]));
        else pt->AddText(Form(" %.2f < #Delta #Phi < %.2f (rad)",DeltaPhi[n],DeltaPhi[n+1]));
        pt->AddText(Form("2.5 < #it{y} < 4"));
      }
      pt->Draw();
      gPad->Modified();
...
//_____________________________________________________________________________
void AliAnalysisMuMuSpectraCapsulePbPb::FitDistvsDphi(Double_t ptMin, Double_t ptMax, const char* particle,const char* subresults, Bool_t draw) const
{
...
void AliAnalysisMuMuSpectraCapsulePbPb::DrawDnDphi(TGraphErrors* distribution, TF1* fit, const char* subresName, Double_t ptMin, Double_t ptMax, Double_t v2, Double_t v2Err) const
{
...
//______________________________________________________________________________
TGraphErrors* AliAnalysisMuMuSpectraCapsulePbPb::PlotFlow(const char* fitResultName, const char* what, Double_t ptBinMin, Double_t ptBinMax, Bool_t divideByBinWidth ) const
{
